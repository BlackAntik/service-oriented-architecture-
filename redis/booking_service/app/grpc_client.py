...
class FlightClient:
    def __init__(self) -> None:
        ...
        self._timeout = float(os.getenv("FLIGHT_GRPC_TIMEOUT_SECONDS", "2.0"))
    ...
    def search_flights(self, origin: str, destination: str, d: date | None) -> list[pb2.Flight]:
        ...
        resp = self._stub.SearchFlights(req, metadata=self._metadata, timeout=self._timeout)
        return list(resp.flights)

    @retry_grpc()
    def get_flight(self, flight_id: uuid.UUID) -> pb2.Flight:
        resp = self._stub.GetFlight(
            pb2.GetFlightRequest(flight_id=str(flight_id)),
    ...
    def release_reservation(self, booking_id: uuid.UUID) -> None:
        self._stub.ReleaseReservation(
            pb2.ReleaseReservationRequest(booking_id=str(booking_id)),
            metadata=self._metadata,
            timeout=self._timeout,
        )
