    ...
    try:
        with session_scope() as session:
            ...
            if existing_passenger:
                # Update name if changed? For now just reuse
        ...
        # If DB save fails, we should try to release reservation (best effort)
        try:
...
def cancel_booking(id: uuid.UUID):
    with session_scope() as session:
        ...
        try:
            ...
            # If release fails, we still cancel the booking?
            # Or should we fail the cancellation?
            # Requirement says: "Вызвать Flight Service (ReleaseReservation) - вернуть места (при решении на 3 балла можно не проверять)."
            # So we can ignore errors here for now, but logging would be good.
            pass
