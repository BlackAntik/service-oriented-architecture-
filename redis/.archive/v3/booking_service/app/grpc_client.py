...
class FlightClient:
    ...
    def search_flights(self, origin: str, destination: str, d: date | None) -> list[pb2.Flight]:
        ...
        resp = self._stub.SearchFlights(req)
        return list(resp.flights)

    def get_flight(self, flight_id: uuid.UUID) -> pb2.Flight:
        resp = self._stub.GetFlight(pb2.GetFlightRequest(flight_id=str(flight_id)))
        return resp.flight

    def reserve_seats(self, flight_id: uuid.UUID, booking_id: uuid.UUID, seats: int) -> pb2.Reservation:
        resp = self._stub.ReserveSeats(pb2.ReserveSeatsRequest(flight_id=str(flight_id), booking_id=str(booking_id), seats=seats))
        return resp.reservation

    def release_reservation(self, booking_id: uuid.UUID) -> None:
        self._stub.ReleaseReservation(pb2.ReleaseReservationRequest(booking_id=str(booking_id)))
