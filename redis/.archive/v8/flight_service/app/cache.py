...
def cache_proto(key_func: Callable[..., str], ttl: int = 300, proto_class: Any = None):
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if cached_json:
                try:
                    # If it's a list (SearchFlightsResponse.flights is repeated)
                    # But here we cache the whole response message usually.
                    # Let's assume func returns a proto message.
                    message = proto_class()
            ...
            try:
                # Convert proto to dict then json
